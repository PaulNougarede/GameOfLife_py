...
def pageAccueil(screen):
    vert = (0, 122, 123) # def des couleurs
    blanc = (255, 255, 255)
    jeu = False

    imageMenuAccueil = pygame.image.load("main/images/menuAccueil.png").convert_alpha() # chargement de l'image de fond
    screen.blit(imageMenuAccueil, (0, 0)) # affichage
    ...
    while running:  # boucle infinie
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:  # si on appuie sur une touche
                if event.key == K_ESCAPE:  # si c'est la touche echap
                    pygame.quit()
                    sys.exit()

        Menu.draw_text("LE JEU DE LA VIE",pygame.font.SysFont("Futura", 150),(255, 255, 255),250,300,screen) # ecriture texte
        ...
        if bouton_rect.collidepoint(pygame.mouse.get_pos()):  # si souris sur le bouton
            if pygame.mouse.get_pressed()[0]:  # si clic
                bouton_rect = pygame.draw.rect(screen, vert, (525, 450, 300, 100))  # "dessin" bouton
                ...
                running = False
                time.sleep(0.07) # pour ne pas que le bouton soit cliqué plusieurs fois
    return jeu
